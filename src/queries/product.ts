...
import { dummyProducts } from "@/constants/dummy-data";
...
export const useProductBycategory = ({ slug }: { slug: string }) => {
  ...
  return useQuery({
    ...
    queryFn: async ({ queryKey }) => {
      ...
      return dummyProducts.filter(p => p.category.slug === slug || p.category.name.toLowerCase() === slug.toLowerCase());
    },
    enabled: !!slug,
  });
};
...
export const useListProducts = (filter: any) => {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (filter) {
        if (filter.category) {
          list = list.filter(p => p.category.slug === filter.category || p.category.name.toLowerCase() === filter.category.toLowerCase());
        }
        if (filter.brand) {
